...
tags:
  ...
  - Splunk Cloud
redirect_from: endpoint/uac_bypass_with_colorui_com_object/
---
...
| stats count min(_time) as firstTime max(_time) as lastTime by dest parent_process_name process_name ImageLoaded signature_id signature Signed user_id 
...
| ----------- | ----------- |--------------|--------------|
| 48.0 | 60 | 80 | The following module $ImageLoaded$ was loaded by a non-standard application $parent_process_name$ on endpoint $dest$ by user $user_id$. |
