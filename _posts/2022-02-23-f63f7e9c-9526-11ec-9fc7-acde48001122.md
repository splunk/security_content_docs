...
| where (process_file_name IN ("wscript.exe", "cscript.exe", "searchprotocolhost.exe", "microsoft.workflow.compiler.exe", "msbuild.exe", "colorcpl.exe", "scrcons.exe", "cmd.exe", "powershell.exe")) AND actor_process_file_name LIKE "%mshta.exe" --finding_report--
```
...
[*source*](https://github.com/splunk/security_content/tree/develop/detections/endpoint/windows_mshta_child_process.yml) \| *version*: **4**