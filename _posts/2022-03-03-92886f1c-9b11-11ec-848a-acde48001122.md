...
| where process_file_name="msbuild.exe" AND (match(actor_process_file_name, /(?i)cscript.exe/)=true OR match(actor_process_file_name, /(?i)wscript.exe/)=true) --finding_report--
```
...
[*source*](https://github.com/splunk/security_content/tree/develop/detections/endpoint/windows_script_host_spawn_msbuild.yml) \| *version*: **3**