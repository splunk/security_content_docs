...
tags:
  ...
  - Splunk Cloud
  - Change
redirect_from: endpoint/detect_excessive_account_lockouts_from_endpoint/
---
...
| tstats `security_content_summariesonly` count min(_time) as firstTime max(_time) as lastTime values(All_Changes.user) as user from datamodel=Change.All_Changes where All_Changes.result="*locked out*" by All_Changes.dest All_Changes.result 
...
[*source*](https://github.com/splunk/security_content/tree/develop/detections/endpoint/detect_excessive_account_lockouts_from_endpoint.yml) \| *version*: **6**