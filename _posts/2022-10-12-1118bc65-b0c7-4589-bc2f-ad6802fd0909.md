...
| where match_regex(process_cmd_line, /(?i)get-recipient/)=true OR match_regex(process_cmd_line, /(?i)new-mailboxsearch/)=true OR match_regex(process_cmd_line, /(?i)new-managementroleassignment/)=true OR match_regex(process_cmd_line, /(?i)new-mailboxexportrequest/)=true --finding_report--
```
...
[*source*](https://github.com/splunk/security_content/tree/develop/detections/endpoint/windows_exchange_powershell_module_usage.yml) \| *version*: **1**