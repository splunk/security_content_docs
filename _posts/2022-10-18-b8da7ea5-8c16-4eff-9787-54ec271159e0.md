---
title: "Windows Rename System Utilities Atbroker exe LOLBAS in Non Standard Path"
...
tags:
  ...
  - Endpoint_Processes
redirect_from: endpoint/windows_rename_system_utilities_atbroker_exe_lolbas_in_non_standard_path/
---
...
The following analytic identifies Atbroker.exe which is a native living off the land binary or script (LOLBAS) within the Windows operating system that may be abused by adversaries by moving it to a new directory. The list of binaries was derived from the https://lolbas-project.github.io site.
...
| where process_name IS NOT NULL AND process_name="atbroker.exe"
| where process_path IS NOT NULL AND match_regex(process_path, /(?i)\\windows\\syswow64/)=false 
...
> :information_source:
> **windows_rename_system_utilities_atbroker_exe_lolbas_in_non_standard_path_filter** is a empty macro by default. It allows the user to filter out any results (false positives) without editing the SPL.
...
[*source*](https://github.com/splunk/security_content/tree/develop/detections/endpoint/windows_rename_system_utilities_atbroker_exe_lolbas_in_non_standard_path.yml) \| *version*: **1**