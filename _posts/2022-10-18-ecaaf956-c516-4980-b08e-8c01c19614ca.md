...
| where process_name IS NOT NULL AND process_name="adplus.exe"
| where process_path IS NOT NULL AND match_regex(process_path, /(?i)\\program files (x86)\\windows kits\\10\\debuggers\\x86/)=false 
