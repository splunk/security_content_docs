...
| stats count min(_time) as firstTime max(_time) as lastTime by user src_ip LogonError user_agent UserId Operation Workload  authentication_method authentication_service
| where count >= 5 
...
| ----------- | ----------- |--------------|--------------|
| 64.0 | 80 | 80 | User $user$ has caused excessive number of SSO logon errors from $src_ip$ using UserAgent $UserAgent$. |
