...
| tstats count min(_time) as firstTime max(_time) as lastTime from datamodel=Web where Web.url IN ("*/setup/setupadministrator.action*", "*/setup/finishsetup.action*") Web.status=200  by Web.http_user_agent, Web.status Web.http_method, Web.url, Web.url_length, Web.src, Web.dest, sourcetype 
...
[*source*](https://github.com/splunk/security_content/tree/develop/detections/web/confluence_data_center_and_server_privilege_escalation.yml) \| *version*: **2**